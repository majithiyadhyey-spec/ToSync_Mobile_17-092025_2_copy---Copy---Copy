...
const EditProjectModal: React.FC<EditProjectModalProps> = ({ project, onClose }) => {
  const { updateProject, getTasksByProjectId, addAuditLog } = useFormworkData();
  const { customers } = useErpData();
  const { currentUser } = useAuth();
  const { t } = useI18n();
  const [name, setName] = useState(project.name);
  const [marking, setMarking] = useState(project.marking);
  const [markingColor, setMarkingColor] = useState(project.markingColor);
  ...
  useEffect(() => {
    const initialCustomer = customers.find(c => c.id === project.clientId);
    if (initialCustomer) {
        setSearchQuery(initialCustomer.name);
    }
  }, [project, customers]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const conflictingTask = projectTasks.find(task => 
        new Date(task.startDate) < new Date(startDate) || new Date(task.deadline) > new Date(endDate)
    );
    if(conflictingTask){
        setError(t('editProjectModal_error_conflict'));
        return;
    }

    updateProject({ ...project, name, marking, client: selectedCustomer.name, clientId, markingColor, startDate, endDate });
    if (currentUser) {
        addAuditLog({
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-lg">
        <form onSubmit={handleSubmit}>
          <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('editProjectModal_title', { projectName: project.name })}</h2>
          ...
          <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-4">
            ...
            <button type="submit" className="px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-500 transition">{t('saveChanges')}</button>
          </div>
        </form>
      </div>
    </div>
    {isAddCustomerModalOpen && <CustomerFormModal customer={{ name: searchQuery, id: '', createdAt:'' }} onClose={() => setIsAddCustomerModalOpen(false)} onCustomerAdded={handleCustomerAdded} />}
    </>
  );
};
